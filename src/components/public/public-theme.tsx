...
import { paletteStyle, type StorePalette } from "@/lib/store-palettes";
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuRadioGroup, DropdownMenuRadioItem } from "@/components/ui/dropdown-menu";

export type PublicTheme = 'light' | 'dark' | 'system';
const ThemeContext = createContext<PublicTheme>('system');
const PaletteContext = createContext<StorePalette>('forest');
export const useStorePalette = () => useContext(PaletteContext);
const SetThemeContext = createContext<(theme: PublicTheme) => void>(() => {});
export const usePublicTheme = () => useContext(ThemeContext);

export function PublicThemeProvider({ initialTheme, palette = 'forest', children }: { initialTheme: PublicTheme; palette?: StorePalette; children: React.ReactNode }) {
  ...
  return <PaletteContext value={palette}><ThemeContext value={theme}><SetThemeContext value={changeTheme}>
    <div data-store-theme={theme} data-store-palette={palette} style={paletteStyle(palette)} className="storefront-theme public-site flex min-h-screen flex-col">{children}</div>
  </SetThemeContext></ThemeContext></PaletteContext>;
}
...
export function PublicThemeSelect() {
  ...
  return <DropdownMenu>
    ...
    <DropdownMenuContent align="end" sideOffset={10} data-store-theme={theme} style={paletteStyle(palette)} className="storefront-theme public-theme-menu">
